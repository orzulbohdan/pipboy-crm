      ...
      <div className="min-h-screen bg-black text-green-500 font-mono flex flex-col justify-center items-center p-4">
        ...
        <div className="border border-green-500 p-6 w-full max-w-sm rounded-sm bg-zinc-900/50 shadow-[0_0_15px_rgba(34,197,94,0.2)]">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            <div>
              <label className="text-xs text-green-600 block mb-1">ЛОГИН ВЫЖИВШЕГО</label>
              <input type="text" placeholder="ИМЯ" value={username} onChange={e => setUsername(e.target.value)} className="w-full bg-black border border-green-700 p-2 text-green-500 focus:outline-none focus:border-green-400 placeholder-green-900" required />
            </div>
            <div>
              <label className="text-xs text-green-600 block mb-1">КОД ДОСТУПА</label>
              <input type="password" placeholder="ПАРОЛЬ" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-black border border-green-700 p-2 text-green-500 focus:outline-none focus:border-green-400 placeholder-green-900" required />
            </div>
            <button type="submit" disabled={loading} className="w-full bg-green-900 border border-green-500 p-2 font-bold hover:bg-green-500 hover:text-black transition-colors uppercase tracking-wider mt-2 disabled:opacity-50">
    ...
    <div className="min-h-screen bg-black text-green-500 font-mono p-4 pb-12 select-none relative overflow-x-hidden">
      ...
      <details className="mb-6 border border-green-700 bg-zinc-900 rounded-sm p-3 text-sm">
        ...
        <form onSubmit={createQuest} className="mt-3 space-y-3">
          ...
          <button type="submit" className="w-full bg-green-900 border border-green-500 p-2 font-bold uppercase hover:bg-green-500 hover:text-black transition-colors">Раздать квест в пустошь</button>
        </form>
      </details>
        ...
        {quests.filter(q => q.status !== 'completed').map((quest) => (
          <div key={quest.id} className="border border-green-700 p-4 bg-zinc-900 rounded-sm hover:border-green-400 transition-colors">
            <div className="flex justify-between items-start gap-4 flex-wrap sm:flex-nowrap">
              ...
              <button onClick={() => completeQuest(quest.id, quest.difficulty)} className="w-full sm:w-auto bg-green-900 border border-green-500 text-green-400 text-xs px-4 py-2.5 uppercase tracking-wider font-bold hover:bg-green-500 hover:text-black transition-all">Выполнить</button>
            </div>
          </div>
        ))}
